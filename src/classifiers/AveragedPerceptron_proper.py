...
averaged_weights_list_positive = []
averaged_weights_list_negative = []
averaged_weights_list_objective = []

averaged_weights_positive = [0]*len(distinct_tokens)
averaged_weights_negative = [0]*len(distinct_tokens)
averaged_weights_objective = [0]*len(distinct_tokens)

# D = [(training_input_vector_list, desired_output)]
# W = [w1, w2, ... wn]
fileName = GENERATED_FILES_DIRECTORY+"averaged_training_perceptron_data_akj"
plotFileName = GENERATED_FILES_DIRECTORY+"averaged_training_perceptron_response_akj"
f1 = open(fileName, "w")
number_of_updates = 0
for i in range(MAX_ITERATIONS):
    ...
    averaged_weights_positive = np.add(averaged_weights_positive, weights_dictionary[POSITIVE_POLARITY])
    averaged_weights_negative = np.add(averaged_weights_negative, weights_dictionary[NEGATIVE_POLARITY])
    averaged_weights_objective = np.add(averaged_weights_objective, weights_dictionary[OBJECTIVE_POLARITY])
                
    temp_average = [x/float(i+1) for x in averaged_weights_positive]
    averaged_weights_list_positive.append(temp_average)
                
    temp_average = [x/float(i+1) for x in averaged_weights_negative]
    averaged_weights_list_negative.append(temp_average)
              
    temp_average = [x/float(i+1) for x in averaged_weights_objective]
    averaged_weights_list_objective.append(temp_average)
    f.close()
...
#Useful for a rainy day
f = open(GENERATED_FILES_DIRECTORY+"averaged_weight_vector_akj", "w")
for weight in weights_dictionary[POSITIVE_POLARITY]:
    f.write(str(weight)+" ")
f.write("\n")
f.close()
#Rainy day ends
...
responseFileData = GENERATED_FILES_DIRECTORY+"averaged_development_perceptron_data_akj"
...
for i in range(MAX_ITERATIONS):
    ...
    accuracy = 0
    #Perceptron response
    myweights_positive = averaged_weights_list_positive[i]
    myweights_negative = averaged_weights_list_negative[i]
    myweights_objective = averaged_weights_list_objective[i]
    responseFile = GENERATED_FILES_DIRECTORY+"averaged_perceptron_response_akj"
