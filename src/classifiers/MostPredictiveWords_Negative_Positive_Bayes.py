...
responseFile = GENERATED_FILES_DIRECTORY+"smoothing_response_"+str(POW)
words_difference_dictionary = {}
...
for key, token_list in words_in_development.iteritems():
    ...
    for word in token_list:
        ...
        if word not in words_difference_dictionary:
            words_difference_dictionary[word] = log_probability_negative - log_probability_positive
        else:
            difference = log_probability_negative - log_probability_positive
            if words_difference_dictionary[word]<difference:
                words_difference_dictionary[word] = difference 
...
words_difference_dictionary = OrderedDict(sorted(words_difference_dictionary.items(), key=lambda x: x[1]))
counter = 0

f = open(GENERATED_FILES_DIRECTORY+"top5neg_pos", "w")
classificationSentimentDictionary = SentimentParser().wordsClassification()
for key, values in words_difference_dictionary.iteritems():
    print key
    if key in classificationSentimentDictionary:
        f.write(key+"\tYES")
    else:
        f.write(key+"\tNO")
    f.write("\n")
