...
NEG_INFINITY = -99999
POW = 2 #Other values are 0,1,2,3 (best),4,5,6
...
words_in_development = wordTokenizer.tokenizeDirectoryAsDictionary(TEST_DATA_DIRECTORY)
words_in_development = OrderedDict(sorted(words_in_development.items(), key=lambda t: t[0]))

responseFile = GENERATED_FILES_DIRECTORY+"jagannathan-arvind_krishnaa.special.response"
...
for key, token_list in words_in_development.iteritems():
    ...
    for word in token_list:
        ...
        probability_word_positive_documents = probabilityOfWordInSeveralDocumentsPutTogether(word, except_positive_training_document_bag_dictionary, total_count)
        log_probability_positive1 = math.log(probability_word_positive_documents) if probability_word_positive_documents > 0 else NEG_INFINITY
        ...
        p+=log_probability_positive - log_probability_positive1
        
        probability_word_negative_documents = probabilityOfWordInSeveralDocumentsPutTogether(word, except_negative_training_document_bag_dictionary, total_count)
        log_probability_negative1 = math.log(probability_word_negative_documents) if probability_word_negative_documents > 0 else NEG_INFINITY
        ...
        n+=log_probability_negative - log_probability_negative1
        
        probability_word_objective_documents = probabilityOfWordInSeveralDocumentsPutTogether(word, except_objective_training_document_bag_dictionary, total_count)
        log_probability_objective1 = math.log(probability_word_objective_documents) if probability_word_objective_documents > 0 else NEG_INFINITY
        ...
        o+=log_probability_objective - log_probability_objective1
