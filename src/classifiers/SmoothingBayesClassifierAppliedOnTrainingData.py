...
NEG_INFINITY = -99999
POW = 7 #Other values are 0,1,2,3 (best),4,5,6
...
words_in_training = OrderedDict(sorted(words_in_training.items(), key=lambda t: t[0]))
...
responseFile = GENERATED_FILES_DIRECTORY+"training_smoothing_response_"+str(POW)
...
for key, token_list in words_in_training.iteritems():
    ...
    f.write(sentiment+"\n")
f.close()
subprocess.call([sys.executable, '../../scorer.py', '../../train.key', responseFile])